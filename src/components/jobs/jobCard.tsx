...
export default function JobCard({ job, index }: { job: JobType, index: number }) {
    ...
    return (
        <>
            <div className="group overflow-hidden rounded-lg bg-white shadow-sm transition-all duration-300 hover:shadow-md transform hover:-translate-y-1">
                <div className="p-5">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => setOpenIndex(index)} className="rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">Expand</button>
                    </div>
                </div>
            </div>
            <JobExpand job={job} open={openIndex === index} setOpen={setOpenIndex} />
        </>
    )
}
