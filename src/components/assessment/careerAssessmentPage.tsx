...
import CareerRecommendations from "./CareerRecommendation";

export default function CareerAssessmentPage({existedForm}: {existedForm?: AssessmentFormData}) {
  const [stage, setStage] = useState(1);
  const [loading, setLoading] = useState(false);
  const [submitted, setSubmitted] = useState(false);
  const {data: session, status, update} = useSession()
  const [careerPaths, setCareerPaths] = useState<{ name: string; description: string }[]>([]);
